...
########## This section is for selecting the model ##########
         # Model is provided by IBM for educational purposes 
         # an API Key is needed to run locally 
my_credentials = {
    "url": "https://us-south.ml.cloud.ibm.com"
}

model_id = 'ibm/granite-4-h-small'
gen_params = {"max_new_tokens": 400}
...
########## Replace it with functional model with API Key ##########
...
def prepare_context_for_llm(query: str, search_results: List[Dict]) ->str:
    ...
    for i, result in enumerate(search_results[:3], 1):
        ...
        food_context.append(f"  - Similarity score: {result['similarity_score']}")
        food_context.append("")
...
def generate_llm_rag_response(query: str, search_results: List[Dict]) -> str:
    ...
    try:
        ...
        prompt = f'''You are a helpful food recommendation assistant. A user is asking for food recommendations, and I've retrieved relevant options from a database.
...
def generate_fallback_response(query: str, search_result: List[Dict]) -> str:
    ...
    return " ".join(response_partssparts)
...
def handle_enhanced_rag_query(collection, query: str, conversation_history: List[Dict]):
...
def handle_enhanced_comparison_mode(collection):
    ...
    query2 = input("Enter your second query: ").strip()
...
Comparison:'''
        ...
        if generated_response and "results" in generated_response:
            ...
            generate_simple_comparison(query1, query2, results1, results2)
...
def show_enhanced_rag_help():
    """Display help for enhanced RAG Chatbot"""
